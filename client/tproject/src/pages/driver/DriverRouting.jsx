...
function DriverRouting() {
  ...
  const handleGetRideRequests = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        console.log("Ride request:", res.data);
        ...
        console.log("list of routes", listsOfRoutes);
  ...
  function groupPendingRidesByDateTimeAndPickUp(data) {
    ...
    for (const ride of pendingRides) {
      ...
      for (const group of groupedData) {
        ...
        const groupDateTime = new Date(
          firstRideInGroup.date + "T" + firstRideInGroup.time
        );

        if (
          ride.pickUp === firstRideInGroup.pickUp &&
          Math.abs(currentDateTime - groupDateTime) <= 5 * 60 * 1000
        ) {
          group.push(ride);
          addedToGroup = true;
          break;
  ...
  function orderDestinationsByDistance(data) {
    ...
    const distances = data.rows[0].elements.map(
    ...
    const sortedDestinations = distanceDestinationPairs.map(
  ...
  const storeRoute = async (originalObj, wayPoints, destination) => {
    try {
      ...
      newObj.names = newObj.names
        .map((name) => (name !== null ? name.toString() : "null"))
        .join(", ");
      ...
      const rId = await http
        .post("/driver/createRoute", newObj)
        .then((res) => {
          if (res.status === 200) {
            ...
            http
              .post(`/driver/chat/${res.data.id}/message`, {
                message:
                  "Hello, I have accepted your ride request, I am on my way.",
              })
              .then((res) => {})
              .catch((err) => {
                console.log(err);
                  ...
                  <CardContent>
                    <Grid container spacing={2}>
                      <Grid item xs={12}>
                        ...
                        <p style={{ margin: 0 }}>
                          Click on any of the route requests and their
                          respective stats will be displayed
                        </p>
                      </Grid>
                      <Grid item xs={6}>
                        {distance && (
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} lg={9} md={7} sm={12}>
                ...
                >
                  {user.rideDirections && (
                    <DirectionsRenderer
                      directions={JSON.parse(user.rideDirections)}
                ...
                <Box className="" display={value == 1 ? "initial" : "none"}>
                  {socket?.connected == true && (
                    <DriverChatBox
                      ...
                      closed={false}
                      abort={() => {handleAbortSocket(user.current_route)}}
                    ></DriverChatBox>
                  )}
                </Box>
                  ...
                  <CardContent>
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={12}>
                        <h3 style={{ margin: 0 }}>Chosen Route Stats: </h3>
                        <p style={{ margin: 0 }}>
                          These are the stats for route: {user.current_route.id}
                        </p>
                      </Grid>
                      <Grid item xs={6}>
                        {user.current_route.distance && (
                          <h5 style={{ margin: 0 }}>
                            Distance: {user.current_route.distance}
                          </h5>
                        )}
                      </Grid>
                      <Grid item xs={6}>
                        {user.current_route.distance && (
                          <h5 style={{ margin: 0 }}>
                            Duration: {user.current_route.duration}
                          </h5>
                        )}
                      </Grid>
                      <Grid item xs={6}>
                        {user.current_route.driver_profit && (
                          <h4 style={{ margin: 0, color: "green" }}>
                            Profit: $
                            {user.current_route.driver_profit.toFixed(2)}
                          </h4>
                        )}
                      </Grid>
                    </Grid>
                ...
                >
                  ...
                  <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                      By aborting route, you will not make the profits and your
                      aborted routes stat will increment
                    </DialogContentText>
                  </DialogContent>
                  <DialogActions>
